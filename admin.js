const WEB_APP_URL = "https://script.google.com/macros/s/AKfycbyFYrPyKEtuDdP6VMDOg-F8XeGwyhZ3TOIJN-m2yzETdEbsAxxGzb-F4VS_t8g2AluWdQ/exec";
let profiles = [];
...
function jsonp(params) {
  return new Promise((resolve, reject) => {
    ...
    script.onerror = () => {
      delete window[callback];
      script.remove();
      reject(new Error('Không tải được dữ liệu từ Google Apps Script'));
    };
...
async function loadProfiles() {
  ...
  try {
    ...
    profiles = res.data || [];
    status.textContent = profiles.length ? '' : 'Chưa có hồ sơ nào.';
    renderList();
  } catch (err) {
    status.textContent = err.message + '. Kiểm tra WEB_APP_URL trong admin.js và Deploy lại Code.gs.';
  }
}
...
function renderList() {
  ...
  profiles.filter(p => JSON.stringify(p).toLowerCase().includes(q)).forEach(p => {
    ...
    tr.innerHTML = `
      ...
      <td>${esc(p.email || '')}</td>
      <td>${esc(p.donVi || '')}</td>
      <td>${esc(p.tongBai || '0')}</td>
      <td>${esc(p.tongDiem || '0')}</td>
      <td><button type="button" class="primary" onclick="exportProfilePDF('${p.profileId}')">Xuất PDF</button></td>`;
...
function buildLLKH(d) {
  ...
  <style>
    ...
    <table><thead><tr><th>TT</th><th>Tên công trình</th><th>Năm công bố</th><th>Tạp chí/NXB</th><th>Loại</th><th>Q</th><th>Vai trò</th><th>DOI/Link</th><th>Điểm</th></tr></thead><tbody>${rows(d.baiBao, [(r,i)=>i+1,'TenBai','Nam','TapChi','Loai','Q','VaiTro','DOI','Diem'], {})}</tbody></table>
